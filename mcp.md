...
```
curl -sS https://smm.table.pw/api/projects \
...
curl -sS "https://smm.table.pw/api/tracks?projectId=978f15b0-..." \
  -H "Authorization: Bearer smm_..."

# 2. Create the track (start: 2026-05-12 09:00 IST = 03:30 UTC)
curl -sS -X POST https://smm.table.pw/api/tracks \
  -H "Authorization: Bearer smm_..." \
...
```
curl -sS -X PATCH https://smm.table.pw/api/tracks/<track-id> \
  -H "Authorization: Bearer smm_..." \
  -H "Content-Type: application/json" \
...
```
curl -sS -X PATCH https://smm.table.pw/api/drafts/<draft-id> \
  -H "Authorization: Bearer smm_..." \
  -H "Content-Type: application/json" \
  -d '{ "status": "scheduled" }'
```
...
```
curl -sS https://smm.table.pw/api/projects -H "Authorization: Bearer smm_..."
```
